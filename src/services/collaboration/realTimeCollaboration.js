...
export class RealTimeCollaborationService {
  ...
  handleConflict(message) {
    ...
    const resolution = this.conflictResolver.resolve(conflict.operations, this.currentUser.id);
  ...
  calculateAssumptionImpact(_assumptionId, _newValue) {
...
class OperationalTransformService {
  ...
  operationsConflict(op1, op2) {
    ...
    return op1.cellId === op2.cellId && Math.abs(op1.timestamp - op2.timestamp) < 1000;
  }
...
class ConflictResolutionService {
  ...
  lastWriterWins(operations, _currentUserId) {
  ...
  mergeChanges(operations, _currentUserId) {
  ...
  manualResolution(operations, _currentUserId) {
    return {
