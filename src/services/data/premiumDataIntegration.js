...
class PremiumDataIntegrationService extends EventEmitter {
  ...
  async getCompanyFundamentals(
    symbol,
    metrics = ['TOTAL_EQUITY', 'TOTAL_DEBT', 'FREE_CASH_FLOW'],
    options = {}
  ) {
    const {
      preferredProvider = 'auto',
  ...
  async getESGData(
    symbol,
    metrics = ['ESG_SCORE', 'CARBON_EMISSIONS', 'GOVERNANCE_SCORE'],
    options = {}
  ) {
    const {
      preferredProvider = 'refinitiv',
      fallbackOrder: _fallbackOrder = ['refinitiv', 'spCapitalIQ']
    } = options;
  ...
  getProvider(name) {
    switch (name.toLowerCase()) {
      ...
      case 'spcapitaliq':
        return this.spCapitalIQ;
      default:
        throw new Error(`Unknown provider: ${name}`);
    }
  }

  async getCachedData(key, ttl = 300000) {
    ...
    return {
      ...
      data: indicators.reduce((acc, indicator) => {
        acc[indicator] = countries.reduce((countryAcc, country) => {
          countryAcc[country] =
            data.data?.filter(
              d => d.identifier.includes(country) && d.identifier.includes(indicator)
            ) || [];
          return countryAcc;
        }, {});
        return acc;
...
class SPCapitalIQClient {
  ...
  async getMarketData(symbols, fields, _options = {}) {
