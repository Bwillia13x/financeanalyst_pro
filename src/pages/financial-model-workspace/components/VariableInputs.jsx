...
const VariableInputs = ({ onVariableChange, onBulkEdit }) => {
  ...
  const [variables, setVariables] = useState({
    assumptions: {
      revenue_growth: { value: 0.052, unit: '%', description: 'Annual revenue growth rate', category: 'Growth' },
      ebitda_margin: { value: 0.235, unit: '%', description: 'EBITDA as % of revenue', category: 'Profitability' },
      tax_rate: { value: 0.25, unit: '%', description: 'Corporate tax rate', category: 'Tax' },
      capex_revenue: { value: 0.035, unit: '%', description: 'CapEx as % of revenue', category: 'Investment' },
      working_capital_revenue: { value: 0.02, unit: '%', description: 'Working capital as % of revenue', category: 'Working Capital' },
      terminal_growth: { value: 0.025, unit: '%', description: 'Long-term growth rate', category: 'Terminal' }
    },
    cost_of_capital: {
      risk_free_rate: { value: 0.032, unit: '%', description: 'Risk-free rate (10Y Treasury)', category: 'Market' },
      market_premium: { value: 0.065, unit: '%', description: 'Equity risk premium', category: 'Market' },
      beta: { value: 1.15, unit: 'x', description: 'Asset beta coefficient', category: 'Risk' },
      cost_of_debt: { value: 0.045, unit: '%', description: 'Pre-tax cost of debt', category: 'Debt' },
      debt_equity_ratio: { value: 0.4, unit: 'x', description: 'Target debt-to-equity ratio', category: 'Capital Structure' }
    },
    market_data: {
      current_share_price: { value: 125.50, unit: '$', description: 'Current trading price', category: 'Market' },
      shares_outstanding: { value: 20.0, unit: 'M', description: 'Shares outstanding', category: 'Equity' },
      market_cap: { value: 2510, unit: '$M', description: 'Current market capitalization', category: 'Valuation' },
      enterprise_value: { value: 2810, unit: '$M', description: 'Current enterprise value', category: 'Valuation' },
  ...
  return (
    <div className="flex flex-col h-full bg-card">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {Object.entries(getVariablesByCategory(filteredVariables(variables[activeSection]))).map(([category, categoryVariables]) => (
          <div key={category} className="mb-6">
            ...
            <div className="space-y-3">
              {categoryVariables.map((variable) => (
                <div key={variable.key} className="p-3 bg-background border border-border rounded-lg">
                      ...
                      {editMode ? (
                        <div className="flex items-center space-x-1">
                          <input
                            ...
                            onChange={(e) => handleVariableChange(activeSection, variable.key, e.target.value)}
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-sm text-muted-foreground ml-1">{variable.unit}</span>
                        </div>
                      )}
                    </div>
                  </div>
