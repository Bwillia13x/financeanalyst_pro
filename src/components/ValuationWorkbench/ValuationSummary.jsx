  ...
  return (
    <div className="grid grid-cols-2 gap-3 text-[13px]">
      <div className="rounded-xl border border-slate-200 p-3">
        ...
        <div className={`text-sm font-medium ${getTvEvColor(tvEvRatio)}`}>{Pct(tvEvRatio, 1)}</div>
      </div>
      <div className="rounded-xl border border-slate-200 p-3">
        ...
        <div
          className={`text-2xl font-bold ${valuation.perShare >= assumptions.price ? 'text-emerald-600' : 'text-rose-600'}`}
        >
          {assumptions.currency} {valuation.perShare.toFixed(2)}
        </div>
        <div className="text-slate-500">Margin of Safety vs Price</div>
        <div
          className={`text-lg font-semibold ${valuation.mos >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}
        >
          {Pct(valuation.mos, 1)}
        </div>
      </div>
