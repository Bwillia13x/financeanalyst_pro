...
import { TrendingUp, TrendingDown, BarChart3, Plus, Minus, Trash2 } from 'lucide-react';
...
const ScenarioModeling = ({
  ...
  const weightedAverageValue = scenarioResults.reduce(
    (sum, result) => sum + result.weightedValue,
    0
  );
  ...
  const ScenarioForm = ({ scenario, index }) => (
    <div className="border rounded-lg p-4 bg-white">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          <label
            htmlFor={`scenario-probability-${index}`}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Probability (%)
          </label>
        ...
        <div>
          <label
            htmlFor={`scenario-revenueGrowth-${index}`}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Revenue Growth (%)
          </label>
        ...
        <div>
          <label
            htmlFor={`scenario-marginImprovement-${index}`}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Margin Change (%)
          </label>
          <input
            ...
            onChange={e =>
              updateScenario(index, 'marginImprovement', parseFloat(e.target.value) || 0)
            }
            className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
      ...
      <div className="flex justify-between items-center">
        <div>
          <p className="text-gray-600">
            Create and compare multiple financial scenarios with different assumptions and
            probabilities.
          </p>
        </div>
        <button
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-xs space-y-1 text-gray-600">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${result.marginImprovement >= 0 ? 'text-green-600' : 'text-red-600'}`}
                      >
                        {result.marginImprovement >= 0 ? '+' : ''}
                        {formatPercent(result.marginImprovement)}
                      </span>
                    </div>
                    <div className="flex justify-between">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-800">
                  {formatCurrency(
                    Math.max(...scenarioResults.map(r => r.enterpriseValue)) -
                      Math.min(...scenarioResults.map(r => r.enterpriseValue))
                  )}
                </div>
      ...
      {scenarios.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm mt-2">
            Click &ldquo;Add Scenario&rdquo; to get started with scenario modeling.
          </p>
        </div>
      )}
    </div>
