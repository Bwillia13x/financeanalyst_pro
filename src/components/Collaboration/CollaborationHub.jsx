...
import { Users, MessageSquare, GitBranch, Share2, Bell, ChevronDown, Settings } from 'lucide-react';
...
export default function CollaborationHub({ isOpen: _isOpen, onClose: _onClose }) {
  ...
  const initializeCollaboration = async () => {
    try {
      ...
      await userPresenceService.joinSession();
  ...
  const setupEventListeners = () => {
    ...
    versionControlService.on('branch:switched', handleBranchSwitch);
  ...
  const loadActiveUsers = async () => {
    try {
      const users = await userPresenceService.getActiveUsers();
  ...
  const loadRecentComments = async () => {
    try {
      const comments = await commentingService.getComments({
        limit: 5,
        sortBy: 'createdAt',
        sortOrder: 'desc'
      });
  ...
  const loadVersionHistory = async () => {
    try {
      const history = await versionControlService.getVersionHistory({ limit: 5 });
  ...
  const handleCommentUpdated = comment => {
    setRecentComments(prev => prev.map(c => (c.id === comment.id ? comment : c)));
  };
  ...
  const handleBranchSwitch = async branchName => {
    try {
      ...
      console.log('Branch switched:', branchName);
    } catch (error) {
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-700 p-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              />
              {isExpanded && <span className="text-white font-medium">Collaboration</span>}
            </div>
            <button
                        ...
                        <div className="flex items-start space-x-2">
                          <img
                            src={
                              ...
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(comment.author.name)}&background=6366f1&color=fff`
                            }
