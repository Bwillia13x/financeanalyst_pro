...
const SecurityDashboard = () => {
  ...
  const getComplianceColor = status => {
    return status === 'compliant' ? 'text-green-600 bg-green-100' : 'text-red-600 bg-red-100';
  };
                ...
                >
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      {compliance.regulation.toUpperCase()}
                    </div>
                  </div>
                  <div
                    className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs ${getComplianceColor(compliance.status)}`}
                  >
                    <Icon size={14} />
          ...
          return (
            <div key={event.id} className="p-6 hover:bg-gray-50">
              <div className="flex items-start justify-between">
                ...
                <div
                  className={`px-3 py-1 rounded-full text-xs font-medium ${getSeverityColor(event.severity)}`}
                >
                  {event.severity.toUpperCase()}
                </div>
              </div>
